...
const { routeQuestion, sectionRowsFrom, rewriteForUpstream, splitCompound, resolveTerm } = await import(
  './ask-router.ts'
);
...
// ---------------------------------------------------------------------------
// The board from the bug report: six terms, twenty one cards, Spring 2028
// holding CS 211, CS 341, CS 374, CS 425, CS 222 and CS 357. "How heavy is my
// Spring 2028 term?" was answered "Nothing is on the board yet" against this
// shape, so the report's own board is checked here rather than a smaller one.
// ---------------------------------------------------------------------------

const WIDE = [
  ['fall-2026', 'Fall 2026', 'Fall', 2026, 0, ['CS 124', 'MATH 221', 'RHET 105', 'CHEM 102']],
  ['spring-2027', 'Spring 2027', 'Spring', 2027, 1, ['CS 128', 'MATH 231', 'CS 173', 'PHYS 211']],
  ['fall-2027', 'Fall 2027', 'Fall', 2027, 2, ['CS 225', 'MATH 241', 'STAT 400']],
  ['spring-2028', 'Spring 2028', 'Spring', 2028, 3, ['CS 211', 'CS 341', 'CS 374', 'CS 425', 'CS 222', 'CS 357']],
  ['fall-2028', 'Fall 2028', 'Fall', 2028, 4, ['CS 421', 'ECE 391']],
  ['spring-2029', 'Spring 2029', 'Spring', 2029, 5, ['CS 497', 'ECE 445']],
];

/** The report's board with the last column in view, which is where it was asked. */
const wideCtx = { ...ctx, planned: plannedFrom(WIDE), selectedCode: null, focusTermId: 'spring-2029' };
/** The same board opened at the first column, so "next term" has somewhere to go. */
const earlyCtx = { ...wideCtx, focusTermId: 'fall-2026' };
...
console.log(`report board   ${plannedFrom(WIDE).length} courses across ${WIDE.length} terms, Spring 2028 holds ${WIDE[3][5].join(', ')}`);
console.log('');
 ...
 */
const SMALL = [
  ['fall-2026', 'Fall 2026', 'Fall', 2026, 0, ['CS 124', 'MATH 221']],
  ['spring-2027', 'Spring 2027', 'Spring', 2027, 1, ['CS 128', 'MATH 231']],
];
const smallCtx = { ...ctx, planned: plannedFrom(SMALL), selectedCode: null, focusTermId: null };

console.log('\n  two term board, nothing open: Fall 2026, Spring 2027\n');
...
console.log('');
process.exit(wrong === 0 && ruleFailures === 0 && emptinessLies === 0 ? 0 : 1);
