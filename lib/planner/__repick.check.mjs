 ...
 *
 * And a Finance student who changed "CPA" to "investment banker" was
 * re-picked personal-finance courses for the goal, lost FSHN 120 to the
 * heavier FSHN 101, and had application-only academies on the board.
 *
...
// --- the pieces on their own ---------------------------------------------------
{
  ...
  const closed = [
    ...
    // "Admission by application only": the academies a finance goal names.
...
function build(st) {
  ...
  const g = A.generatePlan({
    ...
    degreeTotal,
    interests,
    // The career words alone, as the workspace passes them, when the student is given some.
    career: st.career,
  ...
  const input = { context, requirements: L.blocks, board, marks, pools, prior, interests, career: st.career, programName: L.program.name, programCollege: L.program.college, minimumTermCredits: 12, priorCredits, degreeTotal, trackPicks: tracked };
  return { L, g, board, input, tracked, built: R.repickSignature(BALANCED, interests) };
}

const difficultyOf = (code) => grades.get(norm(code))?.difficulty ?? null;

/**
 * A gen-ed pick moves only for a course clearly better on what the student
 * weighted most. A Finance student who asked for the lightest, most relevant
 * courses had FSHN 120 Contemporary Nutrition (difficulty 10) swapped for
 * FSHN 101 (13): heavier, and no nearer investment banking.
 */
function genEdProblems(b, run, priorities, interests = b.input.interests, career = b.input.career) {
  const top = Math.max(priorities.workload, priorities.teaching, priorities.relevance, priorities.coverage, priorities.schedule);
  const scorer = A.qualityScorer({ context, requirements: b.L.blocks, interests, career, programName: b.L.program.name, priorities, carriedCodes: codesOn(b.board) });
  const hit = (code) => (scorer(norm(code)).interest ?? 0) > 0;
  const problems = [];
  for (const c of run.changes.filter((x) => x.kind === 'gened')) {
    const was = difficultyOf(c.from);
    const now = difficultyOf(c.to);
    const lighter = now !== null && (was === null || now < was);
    const goal = hit(c.to) && !hit(c.from);
    const said = `gen-ed ${c.from} (difficulty ${was === null ? '?' : Math.round(was)}) became ${c.to} (${now === null ? '?' : Math.round(now)})`;
    if (priorities.workload === top && was !== null && now !== null && now >= was + 5) problems.push(`${said}, heavier under a workload-led re-pick`);
    if ((priorities.workload === top || priorities.relevance === top) && !(priorities.workload === top && lighter) && !(priorities.relevance === top && goal)) {
      problems.push(`${said}, neither lighter nor nearer the goal`);
    }
  }
  return problems;
}
...
function audit(b, run, priorities) {
  ...
  for (const c of run.changes) {
    ...
    const closed = R.restrictedToOthers(course, context, { programName: L.program.name, programCollege: L.program.college });
  ...
  if (priorities) problems.push(...genEdProblems(b, run, priorities));
  // Nothing a student applies to is ever booked: FIN 391 Investment Banking Academy is "Admission by application only".
  for (const c of run.changes) {
    const application = A.prereqNeedsApplication(context.prereqs.get(norm(c.to))?.text);
    if (application) problems.push(`swapped in ${c.to}, which says "${application}"`);
  }
...
for (const st of STUDENTS) {
  ...
  for (const [label, priorities] of SCENARIOS) {
    ...
    const { problems, total1 } = audit(b, run, priorities);
...
// --- a finance goal changed: CPA, then investment banker ------------------------
{
  // A Finance freshman's board was built for "I want to be a CPA" at balanced
  // priorities; then the career words became "I want to be an investment
  // banker", with workload and relevance both at "matters most", and Re-pick.
  // With one broad finance topic ("financial", "banking" and "investment" as
  // title words) that re-pick chose ACE 240 Personal Financial Planning, ACE
  // 349 Risk Management and Financial Planning Across the Life Cycle and FIN
  // 461 Banking and Financial Regulation for the goal, never FIN 424 Mergers
  // and Acquisition, and swapped the gen-ed FSHN 120 (difficulty 10) for FSHN
  // 101 (13). The CPA build itself booked FIN 391, 392 and 393, academies
  // entered "by application only", as one-credit fillers.
  const st = { name: 'Finance freshman, CPA then investment banker', program: 'bus/finance-bs', studying: 'Finance', after: 'I want to be a CPA', career: 'I want to be a CPA' };
  const b = build(st);
  const career = 'I want to be an investment banker';
  const interests = `${st.studying} ${career}`;
  const priorities = { ...BALANCED, workload: 2, relevance: 2 };
  const title = (code) => byCode.get(norm(code))?.title ?? '?';
  const withDifficulty = (code) => `${code} ${title(code)} (difficulty ${difficultyOf(code) === null ? '?' : Math.round(difficultyOf(code))})`;
  const applicationOnly = (code) => A.prereqNeedsApplication(context.prereqs.get(norm(code))?.text) !== null;
  const start = codesOn(b.board);
  console.log(`\n${st.name}:`);
  const booked = start.filter(applicationOnly);
  if (booked.length) fail(`the CPA build booked ${booked.join(', ')}, entered by application only`);

  const run = R.repickBoard({ ...b.input, interests, career, priorities, lastSignature: b.built });
  for (const c of run.changes) console.log(`  ${c.term.padEnd(12)} ${c.kind.padEnd(9)} ${c.from ? withDifficulty(c.from) : '(added)'} -> ${withDifficulty(c.to)}  [${c.why}]`);
  const { problems } = audit({ ...b, input: { ...b.input, interests, career } }, run, priorities);
  for (const p of problems) fail(`${st.name}: ${p}`);
  // Nothing personal-finance or insurance is a pick for investment banking.
  const personal = run.changes.filter((c) => /\b(personal|wealth|insurance|retirement|benefit|financial planning)\b/i.test(title(c.to)));
  if (personal.length) fail(`personal-finance or insurance courses picked for investment banking: ${personal.map((c) => withDifficulty(c.to)).join('; ')}`);
  // The courses the goal names, where a slot allows: FIN 424, or FIN 463, 418 or 425 when not already there.
  const named = ['FIN 424', 'FIN 463', 'FIN 418', 'FIN 425'].filter((code) => !start.includes(code));
  const brought = run.changes.map((c) => norm(c.to)).filter((code) => named.includes(code));
  if (brought.length === 0) fail(`none of ${named.join(', ')} was picked for investment banking`);
  if (!run.changes.some((c) => norm(c.to) === 'FIN 424')) fail('FIN 424 Mergers and Acquisition was not picked');
  const final = codesOn(run.board);
  if (final.some(applicationOnly)) fail(`an academy entered by application is on the board: ${final.filter(applicationOnly).join(', ')}`);
  console.log(`  ${run.changes.length} changes; brought ${brought.join(', ') || 'none'} for the goal${problems.length || personal.length ? '' : ', clean'}`);

  // FSHN 120 on its own: with ANSC 110 left alone, FSHN 101 is free to take
  // FSHN 120's place, which is how the student saw it. It must not.
  const marks = new Map(b.input.marks);
  marks.delete(byCode.get('ANSC 110')?.id);
  const alone = R.repickBoard({ ...b.input, marks, interests, career, priorities, lastSignature: b.built });
  const fshn = alone.changes.find((c) => norm(c.from) === 'FSHN 120');
  if (fshn) fail(`FSHN 120 (difficulty 10) was swapped for ${withDifficulty(fshn.to)} under workload and relevance "matters most"`);
  if (!codesOn(b.board).includes('FSHN 120')) fail('the CPA board no longer holds FSHN 120; the gen-ed case needs a new course');
}
