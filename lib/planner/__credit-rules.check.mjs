...
const context = {
  ...
  creditRanges,
  bands: meta.bands,
  offeringPublished: offeringsFile ? new Set(rows.map((c) => norm(c.code))) : new Set(), offerings: offeringsFile ? new Map(Object.entries(offeringsFile.courses)) : undefined, offeringTerms: offeringsFile ? offeringsFile.terms : undefined, offeringAliases: offeringsFile?.renumbered ? new Map(Object.entries(offeringsFile.renumbered)) : undefined, languages: languagesFile ?? undefined,
...
for (const summary of summaries) {
  ...
  if (!program) continue;
  totals.programs += 1;
  const result = generatePlan({ requirements: blocks, context, prior: NO_PRIOR, horizon: HORIZON, preferences: PREFS, programId: summary.id, degreeTotal: summary.totalCredits ?? null, programName: summary.name, programCollege: summary.college });
  const placed = [];
