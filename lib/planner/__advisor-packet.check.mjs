...
function printedPages(packet) {
  ...
  // Next term: a table row per course, as tall as its tallest cell.
  if (packet.next) {
    pt += 16 + 12;
    for (const c of packet.next.courses) {
      const cells = [lines(`${c.code} ${c.title} ${c.credits}`, 42), lines(`${c.role} ${c.fills}`, 40), c.backup ? lines(`${c.backup.code} ${c.backup.title} ${c.backup.credits}`, 40) + lines(c.backup.why, 40) : 2];
...
// LAS 100, straight off the card dropdown.
audit(EMMA.name, build(EMMA), { picks: 2, notBackup: { 'RHET 105': 'ESL 115', 'LAS 100': 'LAS 102' } });
