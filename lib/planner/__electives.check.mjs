 ...
 *      international. Ana's Mathematics page prints the row with no subtotal
 *      under it, and it read as all three; it is one pick there too.
 ...
 *      left out of the averages, whichever row they are booked for: a
 *      category takes the track's course where one fits (a pre-med's IB 150
 *      and MCB 150 carry Natural Sciences, PHYS 101 Quantitative Reasoning),
 *      and which of them a knob books as the category's is no measure of how
 *      light its own picks are. Emma's lightest gen-ed picks read 17.1 against
 *      balanced's 15.5 on MCB 150 alone; her picks the track does not ask for
 *      were 5.0 against 6.8. Only a course a track row names alone counts that
 *      way: where the row offers a choice (statistics as STAT 100, STAT 200,
 *      STAT 212 or PSYC 235), which one a list or category picks is exactly
 *      the difficulty choice this check is for. Emma's PHYS 101, her
 *      Quantitative Reasoning pick from the track's "PHYS 101 or PHYS 211"
 *      row, counts among her gen-ed picks under every knob (10.2 balanced,
 *      8.8 lightest), and Aaliyah's HK 264 among her free electives.
 *
...
const { generatePlan, freeElectiveBar, degreeSubjects, admissionGate, arrivalFromWords, interestProfileOf } = await import(join(HERE, 'autoplan.ts'));
...
function picks(r, blocks, career) {
  ...
  // The track's own courses: every one the plan booked for the track, and a
  // course a track row names alone (MCB 150, its lab MCB 151), whichever row
  // booked it. Where a row offers a choice (statistics as STAT 100, STAT 200,
  // STAT 212 or PSYC 235; genetics as MCB 250 or IB 204), the one a list or a
  // category picked is the knob's choice between courses of different
  // difficulty and stays in that pick's average.
  const alone = interestProfileOf(career).tracks.flatMap((t) => t.courses.filter((row) => row.need !== 'suggested').map((row) => row.codes.map(norm).filter((c) => byCode.has(c)))).filter((codes) => codes.length === 1).map(([c]) => c);
  const track = new Set([...r.electives.filter((e) => e.track).map((e) => e.code), ...alone]);
  ...
  for (const t of r.terms) for (const code of t.codes) {
    ...
    if ((kind === 'choose' || kind === 'pool' || kind === 'gened' || kind === 'hours') && track.has(code)) {
      out.track.push(code);
      continue;
    }
...
for (const st of STUDENTS) {
  ...
  for (const [name, priorities] of Object.entries(variants)) {
    ...
    const p = picks(r, st.blocks, st.after);
