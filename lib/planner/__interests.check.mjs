 ...
 *   - "I want to be an investment banker" was one broad finance topic whose
 *     title words ("financial", "banking") made ACE 240 Personal Financial
 *     Planning a course for the goal, and whose course list named FIN 391,
 *     an academy entered by application only, as one to take.
 *
...
const { generatePlan, qualityScorer, prereqNeedsApplication } = await import(join(HERE, 'autoplan.ts'));
...
const HEARD = [
  ...
  ["law school isn't for me, I want finance", ['finance']],
  ...
  ['I want to go to law school. Law school is not for me, I want finance', ['finance']],
  ...
  ['my dad is a lawyer but I want finance', ['finance']],
  ...
  // Finance, heard as the kind of finance work: "I want to be an investment
  // banker" was one broad finance topic whose words brought ACE 240 Personal
  // Financial Planning to an investment-banking goal.
  ['I want to be an investment banker', ['investment banking/corporate finance']],
  ['Investment banking in Chicago', ['investment banking/corporate finance']],
  ['private equity or M&A', ['investment banking/corporate finance']],
  ['corporate finance', ['investment banking/corporate finance']],
  ['I want to be a financial analyst', ['investment banking/corporate finance']],
  ['hedge funds and trading', ['asset management/markets/trading']],
  ['I want to be a quant', ['asset management/markets/trading']],
  ['stocks and investing', ['asset management/markets/trading']],
  ['wealth management', ['financial planning/wealth management/insurance']],
  ['I want to be a financial advisor', ['financial planning/wealth management/insurance']],
  ['personal finance', ['financial planning/wealth management/insurance']],
  ['I want to work in insurance', ['financial planning/wealth management/insurance']],
  ['real estate development', ['real estate']],
  ['I want to invest in real estate', ['real estate']],
  ['commercial banking', ['commercial banking/lending']],
  ['I want to work at a bank', ['commercial banking/lending']],
  ['I want to work in finance', ['finance']],
  ['I want to go into banking', ['finance']],
  ['finance, maybe investment banking', ['finance', 'investment banking/corporate finance']],
  ['I want to be a CPA', ['accounting/CPA']],
  ['venture capital', ['investment banking/corporate finance', 'entrepreneurship/startups']],
  // ...with the refusals and third person the other topics have.
  ['not investment banking, I want real estate', ['real estate']],
  ['investment banking is not for me, I want wealth management', ['financial planning/wealth management/insurance']],
  ['my dad is an investment banker but I want real estate', ['real estate']],
  ['my dad is a banker', []],
  ["I don't want to do finance anymore, I want marketing", ['marketing/advertising']],
  // ...and what is not finance at all.
  ['I still need quant reasoning', []],
  ['can I trade my class for a later section', []],
  ['I keep messing up derivatives in calc', []],
];
...
// The newer topics' courses and title words exist in the catalog, as the
// file's header promises, and the finance ones have run recently.
{
  const rows = read('illinois/index.json');
  const offered = read('illinois/offerings.json').courses;
  const prereqText = read('illinois/prereqs.json');
  const codes = new Set(rows.map((r) => norm(r.code)));
  const esc = (w) => w.replace(/[.*+?^${}()|[\]\\/]/g, '\\$&');
  const FINANCE = ['finance', 'investment-banking', 'markets', 'financial-planning', 'real-estate', 'commercial-banking'];
  for (const id of ['io-psychology-hr', ...FINANCE]) {
    const topic = INTEREST_TOPICS.find((t) => t.id === id);
    const listed = [...(topic?.courses ?? []), ...(topic?.apply ?? [])];
    const missing = listed.filter((c) => !codes.has(norm(c)));
    check(Boolean(topic) && missing.length === 0, `every ${id} course is in public/illinois/index.json`, missing.join(', '));
    const deadWords = (topic?.words ?? []).filter((w) => !rows.some((r) => new RegExp(`\\b${esc(w)}`).test(r.title.toLowerCase())));
    check(deadWords.length === 0, `every ${id} title word starts a word in some title`, deadWords.join(', '));
    if (!FINANCE.includes(id)) continue;
    const dormant = listed.filter((c) => !(offered[norm(c)]?.length > 0));
    check(dormant.length === 0, `every ${id} course has run in a recent term (offerings.json)`, dormant.join(', '));
    // An academy is applied to, never booked: not a course the scorer calls "named for the goal".
    const both = (topic?.apply ?? []).filter((c) => topic.courses.includes(c));
    check(both.length === 0, `${id}: no course is both to book and to apply to`, both.join(', '));
    const notApplied = (topic?.apply ?? []).filter((c) => !prereqNeedsApplication(prereqText[norm(c)]?.text));
    check(notApplied.length === 0, `${id}: every "apply" course says it is entered by application`, notApplied.join(', '));
  }
  // "financial" alone starts a word in all three titles the investment-banking re-pick chose.
  const ib = interestProfile('I want to be an investment banker');
  for (const code of ['ACE 240', 'ACE 349', 'FIN 461', 'FIN 435', 'FIN 434', 'FIN 230']) {
    const title = rows.find((r) => norm(r.code) === code)?.title.toLowerCase() ?? '';
    const hits = ib.words.filter((w) => new RegExp(`\\b${esc(w)}${w.length < 5 ? '\\b' : ''}`).test(title));
    check(hits.length === 0 && !ib.courses.includes(code), `investment banking: ${code} ${title} is not a title hit or a named course`, hits.join(', '));
  }
  check(ib.courses.includes('FIN 424') && same(ib.apply, ['FIN 391']) && !ib.courses.includes('FIN 391'), 'investment banking names FIN 424 and lists FIN 391 to apply to, not to book', `courses ${ib.courses.join(' ')}; apply ${ib.apply.join(' ')}`);
  check(same(interestProfile('real estate and wealth management').apply, ['FIN 395', 'FIN 393']), 'apply lists every heard topic\'s academy, in the order named');
}
...
console.log('\nqualityScorer: the studying answer names no goal');
{
  ...
  // Finance, "I want to be an investment banker": the courses the goal names
  // are named, and the personal-finance and bank-regulation titles the old
  // "financial" and "banking" words reached are not for the goal at all.
  const fin = load('bus/finance-bs');
  const finScore = qualityScorer({ context, requirements: fin.blocks, programName: fin.name, priorities: RELEVANT, ...said('Finance', 'I want to be an investment banker') });
  const GOAL = /named for the goal|what you said you want/;
  for (const code of ['FIN 424', 'FIN 463', 'FIN 418', 'FIN 425']) check(finScore(code).reasons.includes(NAMED), `Finance, investment banker: ${code} is ${NAMED}`, finScore(code).reasons.join('; '));
  for (const code of ['ACE 240', 'ACE 349', 'FIN 461', 'FIN 435']) {
    const reasons = finScore(code).reasons;
    check(!reasons.some((r) => GOAL.test(r)), `Finance, investment banker: ${code} is not a course for the goal`, reasons.join('; '));
  }
  // The academy's title says what the student said, which is true; it is
  // still not a course the goal names, since nobody books it unadmitted.
  check(!finScore('FIN 391').reasons.includes(NAMED), `Finance, investment banker: FIN 391 Investment Banking Academy is not ${NAMED}`, finScore('FIN 391').reasons.join('; '));
