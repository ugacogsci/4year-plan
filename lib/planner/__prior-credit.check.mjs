...
function run(name, programId, { exams = [], transferText = '', transcript = [], saidMore = false, languageYears = null, language = null, admission = null } = {}) {
  const { s, blocks } = load(programId);
  const also = [...examCourses(exams), ...transcript];
  const prior = { ...readPriorCredit(transferText, exams.length, also, saidMore || transcript.length > 0, examElectiveHours(exams)), languageSemesters: languageYears ?? 0, languageName: language };
  const r = generatePlan({ requirements: blocks, context, prior, horizon: HORIZON, preferences: { creditsPerTerm: { min: 12, target: 15, max: 18 } }, programId, degreeTotal: s.totalCredits ?? null, programName: s.name, programCollege: s.college, admissionRoute: admission ? (admissionFile?.colleges?.[admission] ?? null) : null });
