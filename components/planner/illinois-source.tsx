...
const SEASON_WORD = /\b(fall|spring|summer)\s*(20\d\d)\b/i;

/**
 * The student's own timeline where they gave one, the crawled term otherwise.
 *
 * "graduate spring 2029" in their answer is a real constraint and the plan is
 * wrong if it ignores it. Nothing is inferred from silence: with no date in the
 * text the horizon is eight fall and spring terms from the term the schedule
 * crawl covers, and the board says the graduation label is an assumption.
 */
export function readHorizon(
  ...
  const m = SEASON_WORD.exec(timeline);
  const fallback = {
  ...
  const season = (m[1][0].toUpperCase() + m[1].slice(1).toLowerCase()) as SemesterSeason;
  const year = Number(m[2]);
  // A graduation date before the first term is a typo, not a plan.
  if (year < startTerm.year || year > startTerm.year + 8) return fallback;
  return { ...fallback, gradSeason: season, gradYear: year, stated: true };
}
